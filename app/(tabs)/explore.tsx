...
type PieceType = "bonde" | "dronning" | "konge" | "tårn" | "løper";
...
const WhiteLøper = require("@/assets/svg/white/løper.svg");
const BlackLøper = require("@/assets/svg/black/løper.svg");
...
const createInitialBoard = (): BoardState => {
  ...
  // Bishops - white bishops at c1 and j1, black bishops at c12 and j12
  board["c1"] = { type: "løper", color: "white", hasMoved: false };
  board["j1"] = { type: "løper", color: "white", hasMoved: false };
  board["c12"] = { type: "løper", color: "black", hasMoved: false };
  board["j12"] = { type: "løper", color: "black", hasMoved: false };
...
const getPieceSvg = (piece: Piece) => {
  ...
  if (piece.type === "løper") {
    return piece.color === "white" ? WhiteLøper : BlackLøper;
  }
  return null;
};
...
const isSquareUnderAttack = (
  ...
  // Check for attacking løper (can attack on diagonals)
  ...
  for (const [dRow, dCol] of bishopDirections) {
    ...
    while (
      ...
      if (attacker) {
        // Found a piece - check if it's an attacking bishop
        if (attacker.type === "løper" && attacker.color === byColor) {
          return true;
        }
        // Hit a piece, can't attack through it
        break;
...
const checkGameStatus = (
  ...
  lastMove: { from: string; to: string; movedTwoSquares: boolean } | null
): GameStatus => {
...
const calculatePawnMoves = (
  board: BoardState,
  position: string,
  lastMove: { from: string; to: string; movedTwoSquares: boolean } | null = null
): string[] => {
  const piece = board[position];
...
export default function ChessBoardScreen() {
  ...
  const getAllLegalMovesForColor = (
    board: BoardState,
    color: PieceColor,
    lastMove: { from: string; to: string; movedTwoSquares: boolean } | null
    ...
    for (let row = 0; row < BOARD_SIZE; row++) {
      ...
      for (let col = 0; col < BOARD_SIZE; col++) {
        ...
        squares.push(
          ...
          >
            {PieceSvg && <Image source={PieceSvg} style={styles.pieceImage} />}
